...
-- skip     FOR x IN
-- skip         SELECT  marc
-- skip           FROM  authority.record_entry
-- skip           WHERE id > 0
-- skip                 AND NOT deleted
-- skip                 AND id NOT IN (SELECT DISTINCT record FROM authority.simple_heading)
-- skip     LOOP
-- skip         INSERT INTO authority.simple_heading (record,atag,value,sort_value)
-- skip             SELECT record, atag, value, sort_value FROM authority.simple_heading_set(x);
-- skip     END LOOP;
-- skip END;
-- skip $$;
...
-- skip SELECT metabib.reingest_metabib_field_entries(source)
-- skip   FROM metabib.identifier_field_entry mife
-- skip     INNER JOIN config.metabib_field cmf ON cmf.id = mife.field
-- skip   WHERE cmf.field_class = 'identifier'
-- skip     AND cmf.name = 'issn'
-- skip     AND char_length(value) > 9
-- skip ;
...
-- skip SELECT id
-- skip   FROM biblio.record_entry
-- skip   WHERE (
-- skip     (XPATH('//marc:datafield[@tag="901"][1]/marc:subfield[@code="c"]/text()', marc::XML, ARRAY[ARRAY['marc', 'http://www.loc.gov/MARC21/slim']]))[1]::TEXT IS NULL
-- skip   OR
-- skip     (XPATH('//marc:datafield[@tag="901"][1]/marc:subfield[@code="c"]/text()', marc::XML, ARRAY[ARRAY['marc', 'http://www.loc.gov/MARC21/slim']]))[1]::TEXT <> id::TEXT)
-- skip   AND id > -1;
