# OpenILS::Application::BatchEnrich provides CRUD operations for batch.enrich records.
#
...
package OpenILS::Application::BatchEnrich;
...
sub schedule_list {
    ...
    return $e->die_event unless $e->allowed($BATCH_ENRICH);
    ...
    return $_storage->request(
        'open-ils.cstore.direct.batch.enrich.search.atomic',
        $query,
...
__PACKAGE__->register_method(
    ...
    api_name  => 'open-ils.batch-enrich.schedule.list',
    api_level => 1,
    argc      => 1,
    signature =>
        { desc     => <<"          DESC",
...
sub schedule_get {
    ...
    return $e->event unless $e->checkauth;
    return $e->die_event unless $e->allowed($BATCH_ENRICH);
    return $e->retrieve_batch_enrich($id) or return $e->die_event;
}

__PACKAGE__->register_method(
    method    => 'schedule_get',
    api_name  => 'open-ils.batch-enrich.schedule.get',
    api_level => 1,
    argc      => 1,
    signature =>
        { desc     => <<"          DESC",
...
sub schedule_insert {
    ...
    my $schedule = $e->create_batch_enrich($new_schedule)
...
__PACKAGE__->register_method(
    ...
    api_name  => 'open-ils.batch-enrich.schedule.insert',
    api_level => 1,
    argc      => 1,
    signature =>
        { desc     => <<"          DESC",
...
sub schedule_update {
    ...
    my $s = $e->retrieve_batch_enrich($schedule->id) or return $e->die_event;
    if( $s->runner ne $e->requestor->id ) {
        $e->rollback;
        return 0;
    }

    $e->update_batch_enrich($schedule) or return $e->die_event;
    $e->commit;
    $schedule;
...
__PACKAGE__->register_method(
    ...
    api_name  => 'open-ils.batch-enrich.schedule.update',
    api_level => 1,
    argc      => 1,
    signature =>
        { desc     => <<"          DESC",
...
sub schedule_delete {
    ...
    my $s = $e->retrieve_batch_enrich($id) or return $e->die_event;
    $e->delete_batch_enrich($s) or return $e->die_event;
    $e->commit;
    return 1;
}
...
__PACKAGE__->register_method(
    ...
    api_name  => 'open-ils.batch-enrich.schedule.delete',
    api_level => 1,
    argc      => 1,
    signature =>
        { desc     => <<"          DESC",
