...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.loc.gov/MARC21/slim" xmlns:marc="http://www.loc.gov/MARC21/slim" exclude-result-prefixes="marc">

    <!-- This will make an exact copy of the original record -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.loc.gov/MARC21/slim" xmlns:marc="http://www.loc.gov/MARC21/slim" exclude-result-prefixes="marc">
    ...
    <!-- Replace the authority number with another -->
    <xsl:template match="marc:subfield[\@code='0' and text()='(NL-AMISG)00001']">
        <subfield code="0">(NL-AMISG)99999</subfield>
    </xsl:template>

</xsl:stylesheet>
XSLT

$xslt_templates{2}{xslt}=<<XSLT;
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.loc.gov/MARC21/slim" xmlns:marc="http://www.loc.gov/MARC21/slim" exclude-result-prefixes="marc">
    ...
    <xsl:template match="marc:datafield[\@tag='542']/marc:subfield[\@code='m']/text()">closed</xsl:template>
...
sub child_init {
	...
	$_session_batch = OpenSRF::AppSession->create( 'open-ils.batch-update' );
...
sub list {
	...
	my $response = $_session_batch->request( 'open-ils.batch-update.schedule.list', $auth_token)->gather();
	...
	for my $record (@$response) {
		...
		$html .= "
				<tr>
					...
					<td class=batch><a href=\"${report_url}.batch.log?ses=$auth_token\">$state</a></td>
					<td class=batch>
...
sub copy {
	...
	#
	my $response = $_session_batch->request( 'open-ils.batch-update.schedule.get', $auth_token, $id);
	my $object = Fieldmapper::batch::schedule->new($response->gather());
...
sub show {
	...
	$form .= "
		...
		<tr class=batch_show>
			...
			<td class=batch_show_value><a href=\"${fldReportUrl}.batch.log?ses=$auth_token\">$stateLabel</a></td>
		</tr>
		<tr class=batch_show>
...
sub saveNewDocument {
	...
	# save new document
	my $response = $_session_batch->request( 'open-ils.batch-update.schedule.insert', $auth_token, $o0);
	my $newObject = Fieldmapper::batch::schedule->new($response->gather());
...
sub saveExistingDocument {
	...
	#
	my $response2 = $_session_batch->request( 'open-ils.batch-update.schedule.update', $auth_token, $o2);
}
...
sub harddel {
	...
	#
	my $response = $_session_batch->request( 'open-ils.batch-update.schedule.delete', $auth_token, $id);
